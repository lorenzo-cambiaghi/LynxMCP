...
def main() -> int:
    ...
    try:
        ...
        if r.status_code != 200 or r.json() != {"status": "ok"}:
            print(f"[test] FAIL [1/26]: health wrong: {r.status_code} {r.text}")
            return 1
        print(f"[test] OK [1/26] /api/health")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [2/26]: sources status {r.status_code}: {r.text}")
            return 2
        data = r.json()
        if "sources" not in data or not data["sources"]:
            print(f"[test] FAIL [2/26]: empty sources: {data}")
            return 2
        s0 = data["sources"][0]
        if s0["name"] != "demo" or s0["type"] != "codebase":
            print(f"[test] FAIL [2/26]: wrong source shape: {s0}")
            return 2
        if "locked" not in s0:
            print(f"[test] FAIL [2/26]: missing 'locked' field: {s0}")
            return 2
        print(f"[test] OK [2/26] /api/sources: 1 source, lock flag present")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [3/26]: status endpoint failed: {r.status_code}")
            return 3
        st = r.json()
        if st.get("name") != "demo":
            print(f"[test] FAIL [3/26]: status missing name: {st}")
            return 3
        print(f"[test] OK [3/26] /api/sources/demo/status: name=demo")
        ...
        if r.status_code != 404:
            print(f"[test] FAIL [4/26]: unknown source should be 404, got {r.status_code}")
            return 4
        print(f"[test] OK [4/26] /api/sources/UNKNOWN/status: 404")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [5/26]: doctor failed: {r.status_code}")
            return 5
        data = r.json()
        if "results" not in data or "worst_status" not in data:
            print(f"[test] FAIL [5/26]: doctor missing keys: {list(data)}")
            return 5
        if not data["results"]:
            print(f"[test] FAIL [5/26]: doctor results empty")
            return 5
        print(f"[test] OK [5/26] /api/doctor: {len(data['results'])} checks, "
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [6/26]: config endpoint failed: {r.status_code}")
            return 6
        body = r.json()
        if "content" not in body or "demo" not in body["content"]:
            print(f"[test] FAIL [6/26]: config body wrong: {body}")
            return 6
        print(f"[test] OK [6/26] /api/config: returns content + path")
        ...
        r = client.get("/")
        if r.status_code != 200:
            print(f"[test] FAIL [7/26]: dashboard returned {r.status_code}")
            return 7
        html = r.text
        for needle in ("Dashboard", "LynxManager", "demo", "Sources"):
            if needle not in html:
                print(f"[test] FAIL [7/26]: dashboard missing {needle!r}")
                return 7
        print(f"[test] OK [7/26] dashboard: contains expected widgets")
        ...
        for path in ("/playground", "/config", "/integrations", "/sources", "/doctor"):
            r = client.get(path)
            if r.status_code != 200:
                print(f"[test] FAIL [8/26]: {path} returned {r.status_code}")
                return 8
            if "LynxManager" not in r.text:
                print(f"[test] FAIL [8/26]: {path} missing base layout")
                return 8
        print(f"[test] OK [8/26] 5 placeholder pages all 200")
        ...
        for asset, min_size in (("htmx.min.js", 10_000), ("tailwind.min.js", 100_000)):
            ...
            if r.status_code != 200:
                print(f"[test] FAIL [9/26]: {asset} returned {r.status_code}")
                return 9
            if len(r.content) < min_size:
                print(f"[test] FAIL [9/26]: {asset} too small "
                      f"({len(r.content)} < {min_size}) — likely truncated")
                return 9
        print(f"[test] OK [9/26] static assets served")
        ...
        try:
            ...
            if picked == occupied_port:
                print(f"[test] FAIL [10/26]: picked {picked} which was occupied")
                return 10
            if not (occupied_port < picked <= occupied_port + 5):
                # Could be the OS-assigned fallback — accept if it's any
                # different free port
                if picked == occupied_port:
                    print(f"[test] FAIL [10/26]: didn't advance: {picked}")
                    return 10
        finally:
            s.close()
        ...
        if picked != free:
            ...
            print(f"[test] OK [10/26] _find_free_port: advanced to {picked} (preferred {free} reclaimed?)")
        else:
            print(f"[test] OK [10/26] _find_free_port: preferred port honored when free")
        ...
        if r.status_code != 422:
            print(f"[test] FAIL [11/26]: bad JSON should be 422, got {r.status_code}")
            return 11
        if "JSON syntax" not in r.text:
            print(f"[test] FAIL [11/26]: error message unhelpful: {r.text[:100]}")
            return 11
        print(f"[test] OK [11/26] PUT bad JSON: 422 with 'JSON syntax' message")
        ...
        if r.status_code != 422:
            print(f"[test] FAIL [12/26]: bad schema should be 422, got {r.status_code}")
            return 12
        if cfg_path.read_text() != original_content:
            print(f"[test] FAIL [12/26]: file was overwritten despite invalid schema!")
            return 12
        print(f"[test] OK [12/26] PUT bad schema: 422, file untouched")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [13/26]: valid put should be 200, got {r.status_code}")
            return 13
        # Backup file exists
        backup = cfg_path.with_suffix(cfg_path.suffix + ".bak")
        if not backup.exists():
            print(f"[test] FAIL [13/26]: backup not created at {backup}")
            return 13
        # Backup has the original content
        if backup.read_text() != original_content:
            print(f"[test] FAIL [13/26]: backup content doesn't match original")
            return 13
        # New file has the change
        if ".txt" not in cfg_path.read_text():
            print(f"[test] FAIL [13/26]: new content not persisted")
            return 13
        print(f"[test] OK [13/26] PUT valid: 200 + backup + new content saved")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [14/26]: /playground returned {r.status_code}")
            return 14
        for needle in ("Search playground", "pg-source", "demo", "Code-aware", "Graph", "Diff"):
            if needle not in r.text:
                print(f"[test] FAIL [14/26]: playground HTML missing {needle!r}")
                return 14
        print(f"[test] OK [14/26] /playground: form + tabs rendered")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [15/26]: search returned {r.status_code}: {r.text[:200]}")
            return 15
        if "main.py" not in r.text or "score=0.420" not in r.text:
            print(f"[test] FAIL [15/26]: rendered HTML missing expected content: {r.text[:300]}")
            return 15
        print(f"[test] OK [15/26] /api/playground/search: stubbed result rendered")
        ...
        if r.status_code != 400:
            print(f"[test] FAIL [16/26]: empty query should be 400, got {r.status_code}")
            return 16
        if "empty" not in r.text:
            print(f"[test] FAIL [16/26]: error message wrong: {r.text[:200]}")
            return 16
        print(f"[test] OK [16/26] empty query: 400 with clear message")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [17/26]: find_definition returned {r.status_code}: {r.text[:200]}")
            return 17
        # The fallback path uses search_bm25 — result list must contain `main.py`.
        if "main.py" not in r.text:
            print(f"[test] FAIL [17/26]: find_definition rendering missing 'main.py': {r.text[:300]}")
            return 17
        print(f"[test] OK [17/26] /api/playground/find_definition: bm25 fallback rendered")
        ...
        if r.status_code != 400:
            print(f"[test] FAIL [18/26]: get_callers without graph should be 400, got {r.status_code}")
            return 18
        if "graph" not in r.text.lower():
            print(f"[test] FAIL [18/26]: error message should mention graph: {r.text[:200]}")
            return 18
        print(f"[test] OK [18/26] /api/playground/get_callers: 400 (graph layer not enabled)")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [19/26]: /sources returned {r.status_code}")
            return 19
        for needle in ("Sources", "demo", "type: codebase"):
            if needle not in r.text:
                print(f"[test] FAIL [19/26]: /sources missing {needle!r}")
                return 19
        print(f"[test] OK [19/26] /sources: index renders with 'demo'")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [20/26]: /sources/demo returned {r.status_code}")
            return 20
        for needle in ("demo", "Status", "Rebuild index", "build-status"):
            if needle not in r.text:
                print(f"[test] FAIL [20/26]: detail page missing {needle!r}")
                return 20
        print(f"[test] OK [20/26] /sources/demo: detail page rendered")
        ...
        if r.status_code != 404:
            print(f"[test] FAIL [21/26]: unknown source should be 404, got {r.status_code}")
            return 21
        print(f"[test] OK [21/26] /sources/UNKNOWN: 404")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [22/26]: build returned {r.status_code}: {r.text[:200]}")
            return 22
        if "build-status" not in r.text or "job " not in r.text:
            print(f"[test] FAIL [22/26]: build response missing widget shape: {r.text[:300]}")
            return 22
        # Extract job id from the response (format: "job XXXXXXXX")
        import re
        m = re.search(r"job ([a-f0-9]{8})", r.text)
        if not m:
            print(f"[test] FAIL [22/26]: couldn't extract job id from: {r.text[:300]}")
            return 22
        job_id = m.group(1)
        ...
        for _ in range(20):
            ...
            print(f"[test] FAIL [22/26]: job didn't reach 'done' within 1s")
            return 22
        print(f"[test] OK [22/26] /api/sources/demo/build: job {job_id} → done")
        ...
        try:
            ...
            if r.status_code != 409:
                print(f"[test] FAIL [23/26]: locked build should be 409, got {r.status_code}")
                return 23
            if "Locked" not in r.text:
                print(f"[test] FAIL [23/26]: 409 missing 'Locked' message: {r.text[:200]}")
                return 23
        finally:
            lock_mod.is_storage_locked = original
        print(f"[test] OK [23/26] /api/sources/demo/build (locked): 409 with clear message")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [24/26]: /integrations returned {r.status_code}")
            return 24
        for needle in ("Integrations", "Claude Code", "Cursor", "Antigravity",
                       "mcpServers", "Copy snippet"):
            if needle not in r.text:
                print(f"[test] FAIL [24/26]: /integrations missing {needle!r}")
                return 24
        # The snippet must contain the current python executable path
        if sys.executable not in r.text:
            print(f"[test] FAIL [24/26]: snippet doesn't include current python path")
            return 24
        print(f"[test] OK [24/26] /integrations: per-client cards + snippet")
        ...
        if r.status_code != 200:
            print(f"[test] FAIL [25/26]: rules download returned {r.status_code}")
            return 25
        if "attachment" not in r.headers.get("content-disposition", ""):
            print(f"[test] FAIL [25/26]: missing Content-Disposition header")
            return 25
        if "CLAUDE.md" not in r.headers.get("content-disposition", ""):
            print(f"[test] FAIL [25/26]: wrong filename in disposition: "
                  f"{r.headers.get('content-disposition')}")
            return 25
        if "Code Reuse" not in r.text or "search_demo" not in r.text:
            print(f"[test] FAIL [25/26]: rules content unexpected: {r.text[:200]}")
            return 25
        print(f"[test] OK [25/26] /api/integrations/claude/rules: CLAUDE.md downloaded")
        ...
        if r.status_code != 404:
            print(f"[test] FAIL [26/26]: unknown client should be 404, got {r.status_code}")
            return 26
        print(f"[test] OK [26/26] /api/integrations/UNKNOWN/rules: 404")

        print("\n[test] === SUCCESS: UI scaffolding + playground + build + integrations work as expected ===")
        return 0
    finally:
