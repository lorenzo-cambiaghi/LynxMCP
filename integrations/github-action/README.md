...
> what else you should review. It is based on what the code means and how it is
> wired together, not on matching text. It runs entirely inside your CI, so
...
3. for the files your PR changes, ask Lynx two questions:
   - who else uses this? That is what might break (the blast radius),
   - what code is similar? That is what to review alongside it,
...
outside AI service. You need a GitHub account and nothing else (Actions is free
for public repos; private repos get a monthly allowance).
...
2. Set `LYNX_VERSION` (top of the file) to a published release, 1.6.0 or
   newer. The action uses the `/api/v1/graph` and `format=ndjson` endpoints
   introduced in 1.6.0.
...
time. Pin it to a tag or commit if you prefer.
...
has no `node_modules`, `dist` or build artifacts (they are git-ignored), so
those are not indexed. If your repo commits a large vendored or generated tree
...
when the exact key is not found, it restores the most recent cache whose key
...
- changed or added: re-embeds only those,
- removed: drops them.

Full vs. incremental depends only on whether that SHA cache is present, not on
any flag. Even `lynx build` (which runs `update(force=True)`) does a delta
...
| Every later PR | restore cache, then re-embed only the diff's files | seconds to ~1 min |
...
- Branch scoping: a cache is readable from its own branch, the PR's base
  ...
  new branch's first PR restores `main`'s index (it starts warm) and then
  deltas. Parallel branches don't share each other's caches.
- Eviction: GitHub keeps 10 GB of cache per repo (evicted least recently used)
  and drops anything untouched for 7 days. After a long gap, or on a very large
  repo, a run may fall back to a full index.
- Pin the version: `CHUNKER_VERSION` is part of the cache's validity. If a
  newer Lynx bumps it, the next run does a one-time full re-index. That is why
  the workflow pins `LYNX_VERSION`. Bump it deliberately.
- Config changes (paths, `supported_extensions`) invalidate the snapshot and
  trigger a full rebuild. That is expected.
...
- Self-contained (this workflow): build on the runner, cached as above. Best
  for small and medium repos. First PR slow, the rest fast.
- Self-hosted (big monorepos, strict policies): either a self-hosted runner
  (your own machine, registered with GitHub, where Lynx stays indexed), or Lynx
  running as a service on your server (there is a `Dockerfile` in the repo
  root) with the action pointed at it via `LYNX_API`. CI then does almost no
...
heuristically, then resolved fuzzily by Lynx. If the API is unreachable, the
script writes an empty report and does not fail.
...
- [docs/CORAL.md](../../docs/CORAL.md) and [docs/DUCKDB.md](../../docs/DUCKDB.md): SQL and analytics over the same `/api/v1` endpoints.
